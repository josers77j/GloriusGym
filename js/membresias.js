$(document).ready(function () {
  ...
  function cargarDatos(buscar, numRegistros, pagina) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $(".editar-membresia").click(function () {       
          var idMembresia = $(this).data("id");
          var numRegistros = $('#num_reg').val();
          document.getElementById("containerbutton").innerHTML = "";
          crearBoton("editar", "editar", idMembresia, numRegistros);
          //************************************************************************************** */
          // Hacer la petición AJAX para obtener los datos de la membresia a editar
          $.ajax({
            url: "../Controllers/obtener_membresia.php?id=" + idMembresia,
            type: "GET",
            dataType: "json",
            success: function (membresia) {
              // Llenar los campos del formulario con los datos de la membresia a editars
              console.log(membresia);  
              $("#nombre").val(membresia.nombre);
              $("#descripcion").val(membresia.descripcion);
              $("#duracion").val(membresia.duracion);
              $("#costo").val(membresia.costo);
            
              // Cambiar el texto del botón de submit para indicar que se está editando
              $("button[type='submit']").text("Editar");
  
              // Agregar el atributo data-id al formulario para enviar el ID de la membresia a editar
              $("#form_membresia").attr("data-id", idMembresia);
  
            },
            error: function () {
              alert("Error al obtener los datos del membresias");
        ...
        $(".eliminar-membresia").click(function () {
          var idMembresia = $(this).data("id");
          ...
          Swal.fire({
            ...
            if (result.isConfirmed) {
              $.ajax({
                url: "../Controllers/eliminar_membresia.php?id=" + idMembresia,
                type: "GET",
                success: function () {
                  Swal.fire({
                    icon: 'success',
                    title: 'Membresia Eliminado Correctamente',
                    showConfirmButton: false,
                    timer: 1500
                  })
                  // Recargar la tabla de membresias para mostrar los cambios
                  cargarDatos("", numRegistros, 1);
                  $("#cerrarModal").click();
                },
                error: function () {
                  alert("Error al eliminar el membresia");
                },
              });
            }
          })
  ...
  function editarMembresia(idMembresia, numRegistros) {
    var datos = $("#form_membresias").serialize(); // serializa los datos del formulario
  
    $.ajax({
      url: "../Controllers/actualizar_membresia.php?id=" + idMembresia, // archivo PHP para procesar los datos
      type: "GET",
      data: datos,
      success: function (response) {
        var comp = "success";
        ...
        Swal.fire({
          icon: 'success',
          title: 'Membresia Modificado Satisfactoriamente',
          showConfirmButton: false,
          timer: 1500
        })
